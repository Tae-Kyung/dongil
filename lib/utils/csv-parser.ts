...
export function convertCSVToRecord(csvRow: ProductionRecordCSV): ProductionRecord {
  ...
  return {
    ...
    order_no: (csvRow["NO."] ?? csvRow["NO"])?.trim(),
    client: csvRow.거래처?.trim(),
    site: csvRow.현장?.trim(),
    line: csvRow.라인?.trim(),
    registrar: csvRow.등록자?.trim(),
    note: csvRow.비고?.trim(),
    production_date: parseDate(csvRow.일자) ?? parseDate(csvRow.등록일시?.split(' ')[0]),
    production_time: parseTime(csvRow.시간) ?? (csvRow.등록일시 ? parseTime(csvRow.등록일시.split(' ').slice(1).join(' ')) : undefined),
